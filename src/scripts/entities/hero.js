...
class Hero {
  ...
  setupMesh() {
    ...
    this.lifeMeshBack = new THREE.Mesh(
      this.lifeGeometryBack,
      this.lifeMaterialBack
    );
  ...
  move() {
    ...
    for (let i = 0, len = walls.length; i < len; i++) {
      ...
      if (this.mesh.bbox.intersectsBox(wall.bbox)) {
        if (
          this.mesh.bbox.min.x < wall.bbox.max.x &&
          this.mesh.position.x > wall.position.x &&
          this.velocity.x < 0
        ) {
          this.colliding.left = true;
          this.velocity.x = 0;
          this.mesh.position.x -=
            this.mesh.bbox.min.x - wall.bbox.max.x - 0.00000001;
        }
        if (
          this.mesh.bbox.max.x > wall.bbox.min.x &&
          this.mesh.position.x < wall.position.x &&
          this.velocity.x > 0
        ) {
          ...
          this.velocity.x = 0;
          this.mesh.position.x -=
            this.mesh.bbox.max.x - wall.bbox.min.x + 0.00000001;
        }
      }
    }
    ...
    for (let i = 0, len = walls.length; i < len; i++) {
      ...
      if (this.mesh.bbox.intersectsBox(wall.bbox)) {
        if (
          this.mesh.bbox.min.z < wall.bbox.max.z &&
          this.mesh.position.z > wall.position.z &&
          this.velocity.z < 0
        ) {
          this.colliding.up = true;
          this.velocity.z = 0;
          this.mesh.position.z -=
            this.mesh.bbox.min.z - wall.bbox.max.z - 0.00000001;
        }
        if (
          this.mesh.bbox.max.z > wall.bbox.min.z &&
          this.mesh.position.z < wall.position.z &&
          this.velocity.z > 0
        ) {
          ...
          this.velocity.z = 0;
          this.mesh.position.z -=
            this.mesh.bbox.max.z - wall.bbox.min.z + 0.00000001;
        }
      }
    }
  }
  ...
  updateLights() {
    ...
    this.lightDistance =
      Math.sqrt(
        this.velocity.z * this.velocity.z + this.velocity.x * this.velocity.x
      ) * 10;
