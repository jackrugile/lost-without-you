...
class LoseState extends BaseState {
  ...
  activate() {
    ...
    if (this.game.lastLevelPlayed === "level1") {
      newTitle =
        "Mary and Zoey lost their light and couldn’t<br> find each other, or their way out.";
    }
    if (this.game.lastLevelPlayed === "level2") {
      newTitle =
        "As their light source depleted, so did <br>Zoey and Mary’s chance to survive.";
    }
    if (this.game.lastLevelPlayed === "level3") {
      newTitle =
        "Mary and Zoey were overwhelmed by the <br>darkness and couldn’t find safety.";
    }
    if (this.game.lastLevelPlayed === "level4") {
      newTitle =
        "Alone and lost in the dark, Mary and Zoey were unable to navigate to safety, and succumbed to this scary, dark place.";
    }
    ...
    } Failed<br><span>${this.utils.msToString(this.game.lastLevelTime)}</span>`;
