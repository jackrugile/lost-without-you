...
class PlayState extends BaseState {
  ...
  reset() {
    ...
    this.dom.levelTitle.textContent = `Level ${
      this.game.levelManager.levelNames.indexOf(this.game.currentLevel) + 1
    }`;
  ...
  setupDOM() {
    ...
    this.dom.heroAMeterLightBar = document.querySelector(
      ".state-play-hero-a-meter-light-bar"
    );
    this.dom.heroBMeterLightBar = document.querySelector(
      ".state-play-hero-b-meter-light-bar"
    );
    this.dom.heroAMeterTimeBar = document.querySelector(
      ".state-play-hero-a-meter-time-bar"
    );
    this.dom.heroBMeterTimeBar = document.querySelector(
      ".state-play-hero-b-meter-time-bar"
    );
  ...
  update() {
    ...
    if (this.game.heroA.isActive) {
      this.dom.heroAMeterTimeBar.style.transform = `scaleX(${
        1 - this.switchTime / this.switchTimeMax
      })`;
      this.dom.heroBMeterTimeBar.style.transform = "scaleX(1)";
    } else {
      this.dom.heroAMeterTimeBar.style.transform = "scaleX(1)";
      this.dom.heroBMeterTimeBar.style.transform = `scaleX(${
        1 - this.switchTime / this.switchTimeMax
      })`;
    }
